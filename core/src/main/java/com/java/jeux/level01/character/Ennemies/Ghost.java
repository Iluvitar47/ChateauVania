...
public class Ghost extends Enemy {
    ...
    public Ghost(float startX, float startY, int MaxHealth, int AttackDamage, Player player) {
    ...
    @Override
    public void create() {
        String directory = "assets/Characters/Ghost";
        Map<String, Integer> animations = new HashMap<>();
        animations.put("Idle", 7);
        animations.put("Dead", 7);
        animations.put("Hurt", 4);
        animations.put("Walk", 4);

        spriteManager.loadSprites(directory, animations, "directory");
    ...
    public Rectangle getHitBox() {
        if (isFacingRight()) {
            return new Rectangle(position.x +10 , position.y, spriteWidth+10 , spriteHeight+20);
